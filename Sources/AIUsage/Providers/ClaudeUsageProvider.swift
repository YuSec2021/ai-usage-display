...
actor ClaudeUsageProvider: UsageProvider {
    ...
    func load() async -> ProviderResult {
        ...
        // Claude Desktop records plan usage about every five minutes, while the
        // CLI status line updates much more frequently during an active session.
        let staleAfter: TimeInterval = useDesktopSample ? 420 : 120
        let availability: ProviderAvailability = Date().timeIntervalSince(observedAt) > staleAfter
            ? .stale(lastUpdated: observedAt)
            : .ready(lastUpdated: observedAt)
    ...
    private func aggregateTranscripts() -> [DailyUsage] {
        ...
        for case let url as URL in enumerator where url.pathExtension == "jsonl" {
            ...
            var state = ClaudeTranscriptFileState(size: metadata.size, modifiedAt: metadata.modifiedAt)
            for line in text.split(whereSeparator: \.isNewline) {
                guard let lineData = line.data(using: .utf8),
                      let entry = try? JSONDecoder().decode(ClaudeTranscriptEntry.self, from: lineData),
                      let usage = entry.message?.usage ?? entry.usage,
                      let date = CodexUsageProvider.parseDate(entry.timestamp),
                      date >= cutoff else { continue }
                let day = calendar.startOfDay(for: date)
                if let messageID = entry.message?.id {
                    ...
                    state.anonymousUsage[day, default: .zero] = state.anonymousUsage[day, default: .zero] + usage.asUsage
                }
            }
            transcriptStates[url] = state
        }
        ...
        for state in transcriptStates.values {
            for (messageID, value) in state.messages where seenMessages.insert(messageID).inserted {
                buckets[value.day, default: .zero] = buckets[value.day, default: .zero] + value.usage
            }
            for (day, usage) in state.anonymousUsage {
